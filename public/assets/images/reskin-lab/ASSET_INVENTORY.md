# Banana Monkey Assets

Optimized browser-ready artwork for the Banana Monkey first pass.

Original source archive:

`asset-sources/banana-monkey/`

| Game role | Optimized file | Source file | Notes |
| --- | --- | --- | --- |
| Banana Hand follower | `characters/player.png` | `/Users/ba/Downloads/character-monkeyhand.png` | Appears after 9 bananas. |
| Banana Monkey lead | `characters/lead.png` | `/Users/ba/Downloads/character-bananamonkey.png` | Main character from the start. |
| Banana collectible | `characters/collectible.png` | Catch Me If You Can banana asset | Kept from the proven Catch Me game. |
| Bomb hazard | `characters/hazard.png` | Catch Me If You Can bomb asset | Kept from the proven Catch Me game. |
| Shark first chaser | `characters/chaser-a.png` | `/Users/ba/Downloads/character-shark.png` | Replaces the first gorilla role. |
| Alligator second chaser | `characters/chaser-b.png` | `/Users/ba/Downloads/character-alligator.png` | Replaces the second gorilla role. |
| Sunset beach default | `backgrounds/sunset-stage.png` | `/Users/ba/Downloads/sunset-beach-day.png` | Default full-frame stage. |
| Sunny beach alternate | `backgrounds/sunny-stage.png` | `/Users/ba/Downloads/sunny-beach-day.png` | Alternate tuning/debug option. |
| Concept modal | `concept/original-concept.webp` | `/Users/ba/Downloads/background-gameconcept.png` | Optimized modal image. Original PNG remains archived/source-readable. |

Audio:

| Game role | Optimized file | Source file | Notes |
| --- | --- | --- | --- |
| Background music | `public/assets/audio/music/banana-monkey-theme.mp3` | `/Users/ba/Downloads/(8bit) Ellie - Banana Monkey.mp3` | Ellie's 8-bit Banana Monkey theme. |

Do not use Attack of Ziziphus assets or context in this project.
